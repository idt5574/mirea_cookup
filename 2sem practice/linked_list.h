...
public:
    ...
    DoublyLinkedList<D> filter(const std::vector<D>);
    ...
    const DoublyLinkedList<D>& operator +(const DoublyLinkedList<D>&); // Позволяет получить конкатенацию двух списков
    const DoublyLinkedList<D>& operator +(const D&); // Позволяет получить конкатенацию списка и продукта

    const DoublyLinkedList<D>& operator -(const DoublyLinkedList<D>&); // Вычитает из левого списка те объекты, которые присутствуют в правом (если они есть в левом)
    const DoublyLinkedList<D>& operator -(const D&); // Вычитает продукт из списка (если он в списке есть)
