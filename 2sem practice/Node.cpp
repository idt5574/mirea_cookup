...
Node::Node(const Product& other) // Конструктор копирования
{
    data = std::make_shared<Product>(other); // Переопределённый оператор присваивания продукта (см. Product.cpp)
}
